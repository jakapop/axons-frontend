...
<template>
  <div class="leading-normal tracking-normal mt-[60px] md:mt-[50px]">
    <header>
      <nav
        aria-label="menu nav"
        class="bg-white custom-shadow px-1 mt-0 h-auto fixed w-full z-50 top-0 py-2"
      >
        <div class="flex flex-wrap items-center justify-between px-4">
          ...
          <div class="flex flex-1 w-1/3 justify-start text-white px-2">
            ...
            >
              ...
              />
              <img
                class="flex absolute right-[11px] top-[15px] w-4 bg-transparent rounded text-base text-gray-600"
              ...
              >
                <div class="flex flex-col w-full">
                  <div
                    v-for="(farm, index) in state.farms"
                    :value="farm.id"
                    :key="index"
                  ...
                  >
                    ...
                    >
                      <div class="w-full items-center flex">
                        <div
                          class="hover:text-[#9BD226] font-medium text-sm text-[#01893D] truncate"
                        >
                          {{ farm.name }}
                        </div>
                      </div>
                  ...
                  <Menu as="div" class="ml-3 mt-2 relative">
                    <div>
                      <button type="button" @click="handleClick($event)" class="flex">
                        <img v-if="state.notiCount > oldCount" class="bell w-[30px] h-[30px] mr-1 md:mr-0" src="../assets/img/img_landingpage/notifications1.svg"/>
                        <img v-else class="w-[30px] h-[30px]  mr-1 md:mr-0" src="../assets/img/img_landingpage/notifications1.svg"/>
                      </button>
                      <span
                        class="absolute object-right-top"
                        style="right: -5px; top: -22px"
                      >
                        ...
                        <span v-if="state.notiCount > oldCount " class="inline-flex items-center text-red-500 text-[35px]">•</span>
                      </span>
                    </div>
                    <transition
                        ...
                        <div v-if="isMenuOpen==true" >
                          <div  v-if="state.notiToday.length > 0" class="w-custom w-[350px] sm:w-[350px] md:w-[350px] h-[360px] origin-top-right absolute custom-right -right-4 sm:right-0 mt-5 overflow-auto border-[#01893D] border rounded-[8px] shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50" >
                        <div
                          v-for="(noti, index) in state.notiToday"
                          :key="index" :class="noti.readed == 0?'bg-green-50':'bg-white'" class="relative rounded-[16px] flex flex-col justify-between leading-normal"
                        >
                        <div @click="read_noti(noti.id)" class="relative cursor-pointer py-2 p-2 flex flex-row items-center justify-center ">
                          ...
                          <div class="w-full">
                            <p :class="noti.notiStatusText=='ปิด'?'text-[#827C7B]':noti.notiStatusText=='เปิด'?'text-[#01893D]':noti.notiStatusText=='ออนไลน์'?'text-[#154293]':noti.notiStatusText=='ออฟไลน์'?'text-[#D22626]':'text-[#827C7B]'" class="text-xs font-bold mb-2 break-words w-[200px]" >
    ...
    <!-- background opacity -->
    <div v-if="isMenuOpen == true" class="fixed inset-0 z-40 bg-opacity-50 transition-opacity" style="background: rgba(0, 0, 0, 0.2);"></div>
    <!-- end  background opacity -->

    <main>
      <div
        class="flex flex-col md:flex-row rounded-t-[24px] bg-white mt-[54px] md:mt-0 "
      >

        <nav aria-label="alternative nav bg-[#01893D] ">

          <div
            class="md:bg-[#01893D] fixed top-0 md:relative z-10 w-full md:w-40 lg:w-60 content-center "
          >
            <div
            ...
            >
              ...
              >
                <li
                  class="flex-1 mb-0 md:mb-[10px]"
                  v-for="(item, index) in mainNavigation"
                  v-on:click="!item.href ? openSubMenuList() : ''"
                  :key="index"
                >
                <a
                    :href="item.href"
                  ...
                  >
                    ...
                    />
                      ...
                      class="font-medium md:mt-2 lg:mt-0 lg:w-[120px] name-memu text-[10px] md:text-[16px]"
                      :class="[
                        currentRoute() == item.name ||
                        (item.name == 'Dashboard' &&
                          (currentRoute() == 'ManageFarm' ||
                    ...
                    <span v-else-if="item.name == 'OtherPage'"
                      ...
                      >{{ item.label }}</span>
                    <span v-else :class="[currentRoute() == item.name ? 'border-[#01893D] md:border-0 lg:border-r-4 text-[#01893D]': 'text-[#000] md:mb-2 lg:mb-0']"
                    class="font-medium md:mt-2 lg:mt-0 lg:w-[120px] name-memu text-[10px] md:text-[16px]">{{ item.label }}</span>
                  </a>
                  <div v-if="item.subMenu != ''">
                    <ul
                      v-if="state.openSubMenu == true"
                    ...
                    >
                      <li
                        class="ml-0 lg:ml-10 mt-4"
                        v-for="(submenu, index) in item.subMenu"
                        :key="index"
                      >
                      <router-link  :to="submenu.href" class="mx-10 md:mx-0 text-center flex flex-row md:flex-col lg:flex-row  items-center justify-start md:justify-center lg:justify-start mb-4 lg:mb-6" :class="[
                            currentRoute() == submenu.name
                              ? 'md:border-b-4 lg:border-0 md:border-[#01893D]'
                              : '',
                          ]" >
                        <div class="mr-2" >
                          <img  alt="" class="w-[24px] h-[24px]"  :class="[currentRoute() == submenu.name? 'bg-[#01893D]': 'bg-[#000]',submenu.icon,]"/>
                        </div>
                        <span  :class="[
                              currentRoute() == submenu.name
                                ? 'border-[#01893D] md:border-0  text-[#01893D]'
                                : 'text-[#000] md:mb-2 lg:mb-0',
                            ]" class="whitespace-nowrap font-medium lg:mt-0 name-memu text-[10px] md:text-[16px] md:pr-0" >{{ submenu.label }}</span>
                      </router-link>
                        <!-- <a
                          :href="submenu.href"
                          aria-label="dashboard"
